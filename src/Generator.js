...
const { Row } = require('./CellCollections');
const { 
    range, 
    matrix,
    shuffle,
...
module.exports = class Generator {
    constructor(gridSize, numberBlanks) {
        ...
        const blankGrid = Array(gridSize).fill(
     ...
     */
    shuffleColumns() {
        const columnRotation = this.squareSize * Math.floor(Math.random() * this.squareSize);
        if (columnRotation > 0) {
     ...
     */
    shuffleRows() {
        const rowRotation = this.squareSize * Math.floor(Math.random() * this.squareSize);
        if (rowRotation > 0) {
    ...
    removeValues() {
        ...
        while (blanked < this.numberBlanks) {
            ...
            if (!randomCell.empty()) {
                ...
                const validCellValues = this.allDigits.filter(
                    i => randomCell.entryValid(i)
                );
